...
class GphlParameters(BlissWidget):
    ...
    def prompt_parameters(self, field_list):
        logging.getLogger('queue_exec').debug('Parameter XML:\n%s'
                                              % pprint.pformat(field_list))
        ...
        try:
            ...
            self.params_widget = FieldsWidget(fields=field_list, parent=self)

            current_values = self.workflow.get_gphl_parameters()
            self.params_widget.set_values(current_values)
            self.layout().addMultiCellWidget(self.params_widget, 1, 1, 0, 0, Qt.AlignBottom | Qt.AlignLeft)
            self.params_widget.show()
            self.ok_button.setEnabled(True)
            self.params_widget.setEnabled(True)
        except:
            # unblock workflow_hwobj before raising error
            self.workflow.set_gphl_parameters({})
            raise

    def send_parameters(self):
        if self.params_widget is not None:
            params_map = self.params_widget.get_parameters_map()
            self.workflow.set_gphl_parameters(params_map)
        else:
            raise RuntimeError("params_widget is None, can not set parameters")
